...
MIN_NUM_COLUMNS = 3  # Minimum number of columns
MAX_NUM_COLUMNS = 4  # Maximum number of columns
MIN_BUBBLE_AREA = 80
...
STUDENT_INFO_BOX_REGION = (0.05, 0.05, 0.4, 0.30)  # (x_start, y_start, width, height) as ratios
...
RESULT_FOOTER_REGION = (0.3, 0.87, 0.4, 0.15)  # (x_start, y_start, width, height) as ratios for footer area
...
def auto_rotate_image(img: np.ndarray) -> Tuple[np.ndarray, float]:
    """Detect and correct the orientation of a scanned image."""
    logger.info("Auto-rotating image to correct orientation")
    ...
    if lines is not None:
        ...
        if angles:
            ...
            logger.info(f"Detected rotation angle: {angle:.2f} degrees")
    ...
    if abs(angle) < 1.0:
        ...
        if contours:
            ...
            if max_rect:
                ...
                if angle < -45:
                    angle += 90
                logger.info(f"Contour-based rotation angle: {angle:.2f} degrees")
    ...
    if abs(angle) > 1.0:
        ...
        rotated_img = cv2.warpAffine(img, M, (new_width, new_height), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_CONSTANT, borderValue=(255, 255, 255))
    ...
    if not any([student_info.locker_number, student_info.rank, student_info.name]):
        for line in lines:
            ...
            for keyword in rank_keywords:
                if re.search(keyword, line_clean, re.IGNORECASE) and not student_info.rank:
                    student_info.rank = keyword.title()
                    break
            if (re.match(r'^[a-zA-Z\s]+$', line_clean) and 
                len(line_clean) > 3 and 
                not any(keyword.lower() in line_clean.lower() for keyword in ['locker', 'rank', 'name', 'number']) and
                not student_info.name):
                line_clean = re.sub(r'([a-z])([A-Z])', r'\1 \2', line_clean)
...
def detect_bubbles_scanned(thresh: np.ndarray, gray: np.ndarray) -> List[Tuple[np.ndarray, int, int, int, int]]:
    ...
    for contour in contours:
        if isinstance(contour, np.ndarray) and len(contour) > 0:  # Ensure contour is valid
            area = cv2.contourArea(contour)
...
def group_bubbles_scanned(bubble_contours: List[Tuple], num_questions: int) -> List[List[Dict]]:
    """Group bubbles for scanned images with dynamic column detection."""
    logger.info(f"Grouping bubbles for {num_questions} questions")
    
    if not bubble_contours:
        return []
    
    # Dynamically detect number of columns
    ...
    # Take the most populated columns, between MIN_NUM_COLUMNS and MAX_NUM_COLUMNS
    columns = sorted(columns, key=lambda c: len(c["bubbles"]), reverse=True)
    detected_columns = min(max(MIN_NUM_COLUMNS, len(columns)), MAX_NUM_COLUMNS)
    columns = columns[:detected_columns]
    ...
    all_rows = []
    questions_per_column = num_questions // detected_columns
    if num_questions % detected_columns != 0:
...
def process_omr_image(image_data: bytes, answer_key: List[str], num_questions: int, student_id: str, marks_per_mcq: int = 1, passing_percentage: float = 50.0) -> Dict:
    ...
    try:
        ...
        logger.info(f"Image rotation applied: {rotation_angle:.2f} degrees")
        ...
        result["processing_metadata"]["rotation_angle"] = rotation_angle
...
async def process_answer_sheet(
    ...
    try:
        ...
        # Use db_operations instead of db.exams
        exam = await db_operations.find_one("exams", {"examId": examId})
        if not exam:
            raise HTTPException(status_code=404, detail="Exam not found")
        
        # Use db_operations instead of db.solutions
        solution = await db_operations.find_one("solutions", {"examId": examId})
        if not solution:
...
async def batch_process_answer_sheets(
    ...
    try:
        ...
        # Use db_operations to query the exams table
        exam = await db_operations.find_one("exams", {"examId": examId})
        if not exam:
            raise HTTPException(status_code=404, detail="Exam not found")
        
        # Use db_operations to query the solutions table
        solution = await db_operations.find_one("solutions", {"examId": examId})
        if not solution:
            raise HTTPException(status_code=404, detail="Answer key not found for this exam")
...
async def get_processing_config():
    ...
    config = {
        ...
        "layout": {
            ...
            "num_columns": {"min": MIN_NUM_COLUMNS, "max": MAX_NUM_COLUMNS},
